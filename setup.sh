...
sudo tee /etc/apache2/sites-available/test_test.conf >/dev/null <<EOF
<VirtualHost *:80>
    ServerAdmin test@localhost
    DocumentRoot /var/www/test_test
    ErrorLog \${APACHE_LOG_DIR}/error.log
    CustomLog \${APACHE_LOG_DIR}/access.log combined
    <Directory /var/www/test_test>
...
sudo mkdir -p /var/www/test_test

# Enable the site
sudo a2ensite test_test
...
# Create index.html
cat <<EOF | sudo tee /var/www/test_test/index.html >/dev/null
<html>
  <head>
  ...
  <body>
    <h1>Hello World!</h1>
    <p>This is the landing page of <strong>Test</strong>.</p>
  </body>
</html>
EOF

# Create index.php with phpinfo
echo "<?php phpinfo(); ?>" | sudo tee /var/www/test_test/index.php >/dev/null

# Configure .htaccess for URL rewriting
sudo tee /var/www/test_test/.htaccess >/dev/null <<EOF
RewriteEngine On
...
sudo chown -R $USER:$USER /var/www/test_test
